...
router.post("/register", async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!em || !pw) return res.status(400).json({ error: "Email y contraseña requeridos" });

    const exists = await db.user.findUnique({ where: { email: em }, select: { id: true } });
...
router.post("/login", async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const user = await db.user.findUnique({
      where: { email: em },
      select: { id: true, email: true, passwordHash: true, name: true, avatarUrl: true },
    });
