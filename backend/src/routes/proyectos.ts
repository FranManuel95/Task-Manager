...
type Membership =
  | {
      ok: true;
      proj: { id: string; usuarios: string[] | null; creadoPor: string };
    }
...
router.post("/", async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const usuariosArr = Array.isArray(body.usuarios)
      ? (body.usuarios as unknown[])
      : [];
...
router.patch(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!membership.ok)
        return res.status(membership.code).json({ error: "Forbidden" });
...
router.delete(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const email = req.authUser!.email.toLowerCase();
      const { id } = req.params;
      const target = decodeURIComponent(String(req.params.email || ""))
        .trim()
        .toLowerCase();
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (Array.isArray(body.etiquetas))
        data.etiquetas = body.etiquetas as string[];
    ...
    try {
      ...
      try {
        ...
        const emails: string[] = Array.from(
          new Set<string>(
            items.map((r: any) => String(r.actorEmail ?? "")).filter(Boolean),
          ),
        );
        const users = emails.length
          ? await db.user.findMany({
        ...
        const needTaskTitle = items.filter(
          (r: any) =>
            r.entity === "tarea" &&
            r.entityId &&
            !(r.payload as any)?.entityName,
        );
        let taskTitleById: Record<string, string> = {};
        if (needTaskTitle.length) {
          const ids: string[] = Array.from(
            new Set(needTaskTitle.map((r: any) => String(r.entityId))),
          );
          const tareasRows = await db.tarea.findMany({
            where: { id: { in: ids } },
            select: { id: true, titulo: true },
          });
          taskTitleById = Object.fromEntries(
            tareasRows.map((t) => [t.id, t.titulo]),
          );
        }
        ...
        if (needProjectName.length) {
          const ids: string[] = Array.from(
            new Set(needProjectName.map((r: any) => String(r.entityId))),
          );
        ...
        res.json({
          items: items.map((r: any) => {
            ...
            const displayName =
              pld?.entityName ??
              (r.entity === "tarea" && r.entityId
                ? (taskTitleById[String(r.entityId)] ?? null)
                : null) ??
              (r.entity === "proyecto" && r.entityId
                ? (projectNameById[String(r.entityId)] ?? null)
                : null);
...
router.get("/:id/chat/thread/:targetEmail", async (req, res, next) => {
  try {
    ...
    const other = String(targetEmail || "")
      .trim()
      .toLowerCase();
    if (!other)
...
router.get("/:id/chat/threads/:threadId", async (req, res, next) => {
  ...
  (req as any).url =
    `/${req.params.id}/chat/thread/${req.params.threadId}/messages`;
